...
void incorrect_syntax() {
	fprintf(stderr, "Incorrect syntax. Use \"./otp_dec <ciphertext file> <key file> <port>\"\n");
	exit(1);
}
...
int main(int argc, char *argv[]) {
	...
	// This stores the size of the ciphertext file to be decrypted.
	int file_size;

	// This stores the size of the key file. 
	int key_size;

	// This stores the length of the ciphertext and key files combined, with a
	...
	// This stores the server's address that will send us decrypted data.
	...
	// The user must provide a ciphertext file, a key file, and a port number
	// as command line arguments.
	if(argc != 4) { incorrect_syntax(); }

	// Validate that port number provided is a valid port number.
	...
	// This stores the ciphertext file name.
	char *dec_file = malloc(sizeof(char) * (strlen(argv[1])));
	...
	strcpy(dec_file, argv[1]);
	strcpy(key_file, argv[2]);

	// Get and store the size of both the ciphertext file and the key file.
	struct stat st_file;
	struct stat st_key;
	stat(dec_file, &st_file);
	...
	// cannot reliably decrypt the plaintext file. Print an error and exit.
	if(file_size > (key_size)) {
		fprintf(stderr, "The key is not large enough.\n");
		exit(1);
	}

	// This stores the ciphertext file contents. Initialize it.
	...
	// Open the ciphertext file and key files for reading.
	FILE * fp = fopen(dec_file, "r");
	...
	FILE * tmpfile4 = fopen("tmpfile4", "w");
	...
	FILE * tmpfile5 = fopen("tmpfile5", "w");

	// Read in the ciphertext and key file contents.
	...
	// file contents of the ciphertext file.
	...
	// Write the ciphertext file contents.
	fwrite(file_string, strlen(file_string), 1, tmpfile4);

	// Write the '#' character as a division between the ciphertext and key
	// values.
	fwrite(octo_char, 1, 1, tmpfile4);

	// Write the key file contents.
	fwrite(key_string, strlen(key_string), 1, tmpfile4);
	...
	struct stat st_send;
	stat("tmpfile4", &st_send);
	size_to_send = st_send.st_size;

	// If the size_to_send (so the sum of the ciphertext file size with the key
	// file size with the single octothorpe character) is greater than the max
	// file size, then we cannot be sure that the decryption server will 
	// receive all of the data it needs to complete the decryption. The max
	// file size refers to the 3-byte hexadecimal limit given from the 4-byte
	// message preamble.
	if(size_to_send > MAX_FILE_SIZE) {
		fprintf(stderr, "This program cannot reliably decrypt a file of that size!\n");
		exit(1);
	}
	...
	tmpfile4 = fopen("tmpfile4", "r");

	// Read in all of its contents into the send_string variable.
	fread(send_string, size_to_send, 1, tmpfile4);
	...
	fwrite(octo_char, 1, 1, tmpfile5);
	fwrite(bytes, sizeof *bytes, sizeof bytes, tmpfile5);

	// Write the post-preamble message contents to the tmpfile.
	fwrite(send_string, strlen(send_string), 1, tmpfile5);

	// Close the tmpfile for writing.
	fclose(tmpfile5);

	// Reopen the tmpfile for reading.
	tmpfile5 = fopen("tmpfile5", "r");

	// The full message size is the size_to_send plus the 4-byte preamble.
	int full_size = size_to_send + 4;

	// This stores the entire message to be sent to the decryption server.
	// Initialize it.
	char * full_message = malloc(sizeof(char) * full_size);
	for(i = 0; i < strlen(full_message); i++) { full_message[i] = '\0'; }

	// Read in the full message from tmpfile5.
	fread(full_message, full_size, 1, tmpfile5);
	...
	// valid message for the server to parse and decrypt.
	if(full_message[i] != 35) {
		fprintf(stderr, "File not marked for decryption.\n");
		exit(1);
	}

	// Send the full message to the server for decryption.
	int newmessage = send(s, full_message, full_size, 0);

	// We know that the size of the decryption message to receive back from the
	...
	// decrypted message. Initialize it.	
	...
	if(m == -1) {
		fprintf(stderr, "The otp_dec_d server was not found.\n");
		exit(1);
	}

	// Print the decrypted message to stdout, followed by a newline character.
	printf("%s\n", data);
